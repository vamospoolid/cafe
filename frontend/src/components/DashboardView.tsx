...
const DashboardView = () => {
  ...
  return (
    <div className="p-3 sm:p-6 pb-28 sm:pb-16 w-full flex flex-col gap-5 sm:gap-6 max-w-[1600px] mx-auto">
      
      {/* ─── 1. TOP HEADER & SHIFT LIVE STATUS ────────────────────────────── */}
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3 bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-indigo-600 to-indigo-500 text-white flex items-center justify-center shadow-md shadow-indigo-100 shrink-0">
            <Activity size={22} />
          </div>
          <div>
      ...
      {/* ─── 3. ROW 2: TREN PENJUALAN & TIER TARGET HARIAN ────────────────── */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
        
        {/* Sales Chart Card (2 Columns) */}
        <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200/80 p-5 sm:p-6 flex flex-col justify-between shadow-xs">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
            <div>
              <h3 className="text-base font-black text-slate-900 flex items-center gap-2">
                <TrendingUp size={18} className="text-indigo-600" /> Tren Penjualan Real-time
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">Grafik dinamika omzet harian berdasarkan waktu</p>
            </div>
            {/* Toggle Modes */}
            <div className="bg-slate-100 p-1 rounded-xl flex gap-1 self-start sm:self-auto border border-slate-200/60">
              <button
                onClick={() => setChartMode('hourly')}
                className={`px-3 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer ${chartMode === 'hourly' ? 'bg-white text-indigo-600 shadow-xs' : 'text-slate-500 hover:text-slate-800'}`}
              >
                Hari Ini
              </button>
              <button
                onClick={() => setChartMode('weekly')}
                className={`px-3 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer ${chartMode === 'weekly' ? 'bg-white text-indigo-600 shadow-xs' : 'text-slate-500 hover:text-slate-800'}`}
              >
                7 Hari
              </button>
              <button
                onClick={() => setChartMode('monthly')}
                className={`px-3 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer ${chartMode === 'monthly' ? 'bg-white text-indigo-600 shadow-xs' : 'text-slate-500 hover:text-slate-800'}`}
              >
                30 Hari
              </button>
            </div>
        ...
        {/* Bonus Tier & Payment Methods (1 Column) */}
        <div className="lg:col-span-1 flex flex-col gap-4 justify-between">
          
          {/* Target Tier Bonus Omzet Harian */}
          <div className="bg-gradient-to-br from-slate-900 via-indigo-950 to-indigo-900 text-white rounded-2xl p-5 shadow-sm border border-indigo-800/40 flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h4 className="text-xs font-black uppercase tracking-wider text-amber-300">Tier Bonus Omzet</h4>
                    <div className="text-[10px] text-indigo-300">Target per staf full-time</div>
                  </div>
                </div>
              ...
              <div className="mt-4 bg-white/10 p-3.5 rounded-xl border border-white/10 backdrop-blur-xs">
                <div className="flex justify-between items-center text-xs">
                  <span className="text-indigo-200">Status Capaian:</span>
            ...
            <div className="text-[10px] text-indigo-300/80 mt-3 flex items-center gap-1.5">
              <Lightbulb size={12} className="text-amber-400 shrink-0" />
              <span>Dihitung otomatis dari akumulasi omzet harian.</span>
            </div>
          </div>

          {/* Payment Methods Breakdown */}
          <div className="bg-white rounded-2xl border border-slate-200/80 p-4 sm:p-5 flex flex-col gap-3 shadow-xs">
            ...
            <div className="flex flex-col gap-2">
              {paymentChartData.length === 0 ? (
                ...
                paymentChartData.map((item, index) => {
                  ...
                  return (
                    <div key={item.name} className="flex justify-between items-center bg-slate-50 p-2.5 rounded-xl border border-slate-100 text-xs">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 p-5 flex flex-col justify-between shadow-xs">
          ...
          <div className="flex flex-col gap-2 max-h-60 overflow-y-auto pr-1">
            {(!summary.crewOnDuty || summary.crewOnDuty.length === 0) ? (
              <div className="flex flex-col items-center justify-center py-8 text-slate-400 text-xs bg-slate-50 rounded-xl border border-slate-100">
                <Users size={24} className="text-slate-300 mb-1" />
                <span>Belum ada staf yang presensi masuk hari ini.</span>
              </div>
            ) : (
              summary.crewOnDuty.map((att: any) => {
                ...
                return (
                  <div key={att.id} className="flex justify-between items-center bg-slate-50 p-2.5 rounded-xl border border-slate-100 text-xs hover:border-indigo-100 transition-colors">
                    <div className="flex items-center gap-2.5">
          ...
          <div className="flex flex-col gap-2 max-h-60 overflow-y-auto pr-1">
            {(!summary.lowStockProducts || summary.lowStockProducts.length === 0) ? (
              ...
              summary.lowStockProducts.map((p: LowStockProduct) => (
                <div key={p.id} className="flex items-center justify-between p-2.5 bg-rose-50/50 rounded-xl border border-rose-100 text-xs hover:bg-rose-50 transition-colors">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-lg bg-white flex items-center justify-center text-rose-500 font-bold border border-rose-100 shrink-0">
