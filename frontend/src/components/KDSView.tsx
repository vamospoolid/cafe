...
const KDSView = () => {
  ...
  useEffect(() => {
    ...
    socket.on('order:paid', fetchKDSOrders); // Just in case paid clears something
  ...
  const queueCount = orders.filter(o => o.kdsStatus === 'Pending').length;
  const cookingCount = orders.filter(o => o.kdsStatus === 'Cooking').length;
  const readyCount = orders.filter(o => o.kdsStatus === 'Ready').length;

  return (
    <div className="p-6 h-full flex flex-col bg-slate-50 text-slate-800 overflow-y-auto">
      {/* KDS Header */}
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">Layar khusus area dapur untuk memantau pesanan masuk secara real-time</p>
        </div>
        <div className="flex gap-3">
          <button 
            className="btn bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-indigo-750 shadow-sm rounded-xl px-4 flex items-center gap-2 transition-all active:scale-95 text-xs font-bold"
          ...
          {lastServed && (
            <button 
              className="btn bg-amber-50 hover:bg-amber-100 border border-amber-200 text-amber-800 shadow-sm rounded-xl px-4 flex items-center gap-2 transition-all active:scale-95 text-xs font-bold"
              onClick={() => handleUndoStatus(lastServed.id)}
              title={`Recall ${lastServed.orderNumber}`}
            >
              <RotateCcw size={14} /> Recall Meja {lastServed.table?.tableNo || 'TA'} ({lastServed.orderNumber})
            </button>
          )}
          <button 
            className="btn bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 shadow-sm rounded-xl px-5 flex items-center gap-2 transition-transform active:scale-95 text-xs font-bold" 
      ...
      <div className="flex-1">
        {orders.length === 0 ? (
          <div className="bg-white rounded-3xl border border-slate-200/60 py-16 flex flex-col items-center justify-center text-slate-400 w-full min-h-[350px] shadow-sm">
            ...
            <p className="text-sm text-slate-400 mt-1">Tidak ada pesanan aktif saat ini.</p>
          </div>
        ) : (
